...
describe.sequential("P5.2 subscription lifecycle on real PostgreSQL", () => {
  ...
  it("16a expires a due ACTIVE subscription inline with worker identity and one audit", async () => {
    const fixture = await grantFixture();
    const dueAt = date("2026-09-30T12:00:00.000Z");
    clock = date("2026-10-02T12:00:00.000Z");
    await q("UPDATE subscriptions SET current_period_end=$1 WHERE id=$2", [
      dueAt,
      fixture.subscription.id,
    ]);
    const result = await repository().grantSubscription(
      ...
      context("inline lifecycle grant"),
    );
    expect(result.kind).toBe("OK");
    const expired = await q<Record<string, unknown>>(
    ...
    const transition = (
      await q<Record<string, unknown>>(
        "SELECT transition_revision,from_state,to_state,source,source_event_id,actor_type,reason,occurred_at FROM subscription_transitions WHERE subscription_id=$1 ORDER BY transition_revision DESC LIMIT 1",
        [fixture.subscription.id],
      )
    ).rows[0];
    expect(transition).toMatchObject({
      ...
      to_state: "EXPIRED",
      source: "JOB",
      source_event_id: subscriptionLifecycleJobIdentity({
      ...
      actor_type: "SYSTEM",
      reason: "ACTIVE_PERIOD_ENDED",
      occurred_at: dueAt,
    });
    expect(
      (await auditRows(fixture.subscription.id)).filter(
        (row) => row.action === "SUBSCRIPTION_EXPIRED",
      ),
    ).toHaveLength(1);
  ...
  it("16b fails closed for malformed SUSPENDED origin", async () => {
    const fixture = await grantFixture();
    await q(
      "UPDATE subscriptions SET state='SUSPENDED',state_revision=2,suspended_at=$2 WHERE id=$1",
      [fixture.subscription.id, clock],
    );
    const result = await repository().grantSubscription(
      {
        accountId: fixture.accountId,
      ...
      context("malformed suspended grant"),
    );
    expect(result).toEqual({
      kind: "REJECTED",
      code: "SUBSCRIPTION_CORRUPTED",
    });
    expect((await subscriptionRow(fixture.subscription.id))?.state).toBe(
      "SUSPENDED",
    );
  ...
  it("16c materializes due GRACE to PAST_DUE and still blocks grant", async () => {
    const fixture = await grantFixture();
  ...
  it("16d keeps valid SUSPENDED PAST_DUE origin blocking without corruption", async () => {
    const fixture = await grantFixture();
    ...
    const result = await repository().grantSubscription(
      ...
      context("past due suspended grant"),
    );
    expect(result).toEqual({
      kind: "REJECTED",
      code: "SUBSCRIPTION_ALREADY_EXISTS",
  ...
  it("16e worker and grant race materializes expiry exactly once", async () => {
    clock = date("2026-09-01T12:00:00.000Z");
    const dueAt = date("2026-09-05T12:00:00.000Z");
    const fixture = await grantFixture({ end: dueAt });
    const nextPlanRevisionId = await planRevision();
    clock = date("2026-09-06T12:00:00.000Z");
    const lifecycle = createP5SubscriptionLifecycleRepository(db);
    const [worker, grant] = await Promise.all([
      lifecycle.processDue({
        now: clock,
        batchSize: 10,
        correlationId: randomUUID(),
      }),
      repository().grantSubscription(
        {
          accountId: fixture.accountId,
          planRevisionId: nextPlanRevisionId,
          currentPeriodEnd: future,
        },
        context("worker grant race"),
      ),
    ]);
    expect(worker.corrupted).toBe(0);
    expect(grant.kind).toBe("OK");
    expect(
      (
        await q<{ count: string }>(
          "SELECT count(*)::text AS count FROM subscription_transitions WHERE subscription_id=$1 AND to_state='EXPIRED'",
          [fixture.subscription.id],
        )
      ).rows[0]?.count,
    ).toBe("1");
