...
await run("C1-30", "parallel different dialogues remain independent", async () => { const f = await fixture(); try { const second = f.worker.addTab(88, "second-dialogue"); const [one, two] = await Promise.all([start(f), f.worker.popup({ type: "SA_WORK_START", store_id: f.store.id, tab_id: 88, confirm_change: false, start_intent_id: "second" })]); assert.equal(one.ok, true); assert.equal(two.ok, true); assert.ok([1, 2].includes(healthCalls(f).length), "matching parallel admissions may share their in-flight Health transport"); } finally { await close(f); } });
await run("C1-31", "parallel different stores remain isolated", async () => { const f = await fixture(); try { const second = await f.worker.popup({ type: "SA_STORE_SAVE", store: { marketplace: "wildberries", name: "WB second", credentials: { token: "SECOND" } } }); f.worker.addTab(89, "store-dialogue"); const [one, two] = await Promise.all([start(f), f.worker.popup({ type: "SA_WORK_START", store_id: second.store.id, tab_id: 89, confirm_change: false, start_intent_id: "store-second" })]); assert.equal(one.ok, true); assert.equal(two.ok, true); assert.ok([1, 2].includes(healthCalls(f).length), "matching parallel admissions may share their in-flight Health transport"); } finally { await close(f); } });
...
await runRB("RB-24", "unrelated dialogue remains parallel", async () => { const f = await rebindFixture(); try { const second = f.worker.addTab(88, "unrelated-dialogue"); const [one, two] = await Promise.all([rebindStart(f), f.worker.popup({ type: "SA_WORK_START", store_id: f.store.id, tab_id: 88, confirm_change: false, start_intent_id: "rb-unrelated" })]); assert.equal(one.ok, true); assert.equal(two.ok, true); assert.ok([1, 2].includes(healthCalls(f).length), "matching parallel admissions may share their in-flight Health transport"); } finally { await close(f); } });
