...
async function resume(f) { return f.worker.popup({ type: "OZ_WORK_RESUME", conversation_key: f.key, tab_id: f.worker.tabId }); }
...
console.log(JSON.stringify({ status: failureBatch.length ? "FAIL" : "PASS", scope: "C2.3-C1_ONLINE_WORK_START_RESUME_ADMISSION", cases, failureBatch, executionAuthority: false }, null, 2));
if (failureBatch.length) process.exitCode = 1;
