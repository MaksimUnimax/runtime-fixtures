...
await run("C1-10", "revoked/obsolete generation denied", async () => { const f = await fixture({ workAllowed: false }); try { await deniedStart(f); } finally { await close(f); } });
...
await run("C1-14", "Health UNAVAILABLE denied", async () => { const f = await fixture({ health: "unavailable" }); try { await deniedStart(f); } finally { await close(f); } });
await run("C1-15", "expired Health denied", async () => { const f = await fixture({ healthOverrides: { observedAt: new Date(Date.now() - 16 * 60_000).toISOString(), expiresAt: new Date(Date.now() - 1000).toISOString() } }); try { await deniedStart(f); } finally { await close(f); } });
...
await run("C1-17", "Health network failure denies online admission", async () => { const f = await fixture({ health: "network" }); try { await deniedStart(f); } finally { await close(f); } });
...
await run("C1-42", "production-default UNAVAILABLE yields no Start", async () => { const f = await fixture({ health: "unavailable" }); try { await deniedStart(f); } finally { await close(f); } });
...
for (const [id, health, overrides] of [["RB-03", "pass", { status: "DENY" }], ["RB-04", "unavailable", {}], ["RB-05", "network", {}], ["RB-06", "tampered", {}]]) await runRB(id, "confirmed rebind denial leaves source binding and Work unchanged", async () => { const f = await rebindFixture({ health, healthOverrides: overrides }); try { const r = await rebindStart(f); assert.equal(r.ok, false, JSON.stringify(r)); await assertRebindUnchanged(f); } finally { await close(f); } });
