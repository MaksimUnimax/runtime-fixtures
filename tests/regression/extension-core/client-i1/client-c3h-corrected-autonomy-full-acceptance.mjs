...
await test("AUT-13", "ordinary Ozon command executes offline", async () => { const f = await fresh({ dialogue: "aut13" }); await h.start(f); const before = f.worker.controlNetwork.length; await h.execute(f); await h.waitDelivering(f); assert.equal(f.worker.network.length, 1); assert.equal(f.worker.controlNetwork.length, before); });
