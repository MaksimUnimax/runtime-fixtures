...
describe.sequential(
  ...
  () => {
    ...
    it("keeps exact state, surface, browser, profile, incident, notification, and scheduler dimensions", async () => {
      ...
      // Keep SQL defaults on the same fixed clock as the diagnostics reader.
      await runtime.query(
        "UPDATE health_notification_intents SET created_at=$1,updated_at=$1,next_attempt_at=$1 WHERE dedup_key IN ('diag-open','diag-recovered')",
        [new Date("2026-09-19T11:30:00.000Z")],
      );
