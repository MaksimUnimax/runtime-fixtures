...
async function hydrateTarget(runtime: DatabaseRuntime, row: Row) {
  ...
  const evidenceRows = await runtime.query<Row>(
    `SELECT evidence_id AS "evidenceId",rule_id AS "ruleId",classification,sha256 FROM health_evidence_references WHERE run_id=$1 ORDER BY contour_key,evidence_id LIMIT 32`,
    [row.run_id],
  );
