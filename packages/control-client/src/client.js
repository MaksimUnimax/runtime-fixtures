...
(() => {
  ...
  function healthRequest(detectedAi, credentials, authority) { return { healthTransportVersion: "health_transport_v1", bootstrap: bootstrapRequest(detectedAi, credentials, authority) }; }
  function healthContextFromBootstrap(payload) { if (payload?.contractVersion !== "control_plane_v2" || payload?.ai?.status !== "RESOLVED") return null; return { accountId: payload.account.id, contractVersion: payload.contractVersion, configVersion: payload.configVersion, ai: { family: payload.ai.detected.family, surface: payload.ai.detected.surface, variant: payload.ai.detected.variant, profileKey: payload.ai.profile.profileKey, revision: payload.ai.profile.revision, scopeVariant: payload.ai.profile.scopeVariant, contentSha256: payload.ai.profile.contentSha256 } }; }
  async function acquireSignedHealthAuthority(options = {}) {
    ...
    const authority = clone(state.authority), result = await request("/v1/health-authority", { method: "POST", headers: { Authorization: `Bearer ${state.credentials.accessToken}` }, body: healthRequest(options.detectedAi, state.credentials, authority) });
    ...
    if (verified.payload.status === "PASS") { const expected = healthContextFromBootstrap(authority?.payload); if (!expected || verifier.canonicalJson(expected) !== verifier.canonicalJson(verified.payload.context)) throw error("HEALTH_CONTEXT_MISMATCH"); }
